...
const TasksManagement: React.FC = () => {
  ...
  const handleUpdateTask = async (values: any) => {
    ...
    try {
      await omind_put(`/api/v1/scheduled-tasks/${currentTask.id}`, values);
  ...
  const handleToggleTask = async (task: ScheduledTask) => {
    ...
    try {
      await omind_post(`/api/v1/scheduled-tasks/${task.id}/${action}`);
      ...
      onOk: async () => {
        ...
        try {
          for (const task of disabledTasks) {
            try {
              await omind_post(`/api/v1/scheduled-tasks/${task.id}/enable`);
              successCount++;
            } catch (error) {
              failCount++;
      ...
      onOk: async () => {
        ...
        try {
          for (const task of enabledTasks) {
            try {
              await omind_post(`/api/v1/scheduled-tasks/${task.id}/disable`);
              successCount++;
            } catch (error) {
              failCount++;
  ...
  const handleRunTask = async (task: ScheduledTask) => {
    try {
      const response = await omind_post(`/api/v1/scheduled-tasks/${task.id}/trigger`);
  ...
  const handleDeleteTask = (task: ScheduledTask) => {
    modal.confirm({
      ...
      onOk: async () => {
        try {
          await omind_del(`/api/v1/scheduled-tasks/${task.id}`);
