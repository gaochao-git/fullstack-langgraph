...
const CpuResourceVerification = () => {
  ...
  const handleSubmit = async () => {
    ...
    if (!targetPercent || targetPercent < 1 || targetPercent > 100) {
      message.error('请输入有效的CPU占用率 (1-100)');
      return;
    }
    ...
    try {
      ...
      if (result.data.success) {
        message.success(`验证任务已提交，任务ID: ${result.data.task_id}`);
        // 重置表单
        setTargetPercent(30);
        setDuration(300);
        setHostIpList([]);
  ...
  return (
    <div>
      <Card title="CPU资源验证配置" style={{ marginBottom: 24 }}>
        <Row gutter={[16, 16]}>
          <Col span={24}>
          ...
          <Col span={24}>
            ...
            <Switch
              ...
              onChange={setForceExecution}
              checkedChildren="强制执行"
              unCheckedChildren="跳过执行"
            />
            <div style={{ marginTop: '8px', color: '#666', fontSize: '12px' }}>
              {forceExecution 
                ? '如果同一主机有正在运行的验证任务，将强制终止旧任务并执行新任务' 
                : '如果同一主机有正在运行的验证任务，将跳过该主机的验证'}
            </div>
          </Col>
      ...
      <Modal
        title={`主机 ${selectedHostIp} 的CPU验证历史记录`}
...
export default CpuResourceVerification;