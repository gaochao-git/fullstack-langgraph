...
export const sopProblemRuleApi = {
   ...
   * 获取SOP问题规则列表
   */
  async getRules(params: SOPProblemRuleQueryParams) {
   ...
   * 创建SOP问题规则
   */
  async createRule(data: SOPProblemRuleRequest) {
   ...
   * 更新SOP问题规则
   */
  async updateRule(id: number, data: Partial<SOPProblemRuleRequest>) {
   ...
   * 获取Zabbix监控项列表
   */
  async getZabbixItems(params?: { search?: string; limit?: number }) {
