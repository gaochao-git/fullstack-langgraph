...
# 默认系统提示词
DEFAULT_SYSTEM_PROMPT = """# 智能运维诊断系统 - 主控智能体 v2.0
...
### 工具配置建议
为了确保子智能体正常工作，主智能体需要配置以下基础工具：
- **系统工具**: get_current_time, write_todos, get_documents_content
- **SSH工具**: 至少包含基础的命令执行工具
- **监控工具**: Zabbix 相关工具（如果需要监控分析）
- **日志工具**: Elasticsearch 相关工具（如果需要日志分析）
...
### 子智能体工具继承
子智能体会从主智能体继承工具，但只使用它们需要的子集：
- log-analyzer: 主要使用日志和SSH工具
- monitor-analyzer: 主要使用监控、SSH和数据库工具
- alert-correlator: 主要使用告警和监控工具
- change-analyzer: 主要使用SSH和文档工具
