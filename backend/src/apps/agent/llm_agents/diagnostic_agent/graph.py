...
async def create_diagnostic_agent(config: RunnableConfig):
    ...
    # 使用统一的诊断智能体（集成记忆 + 子智能体）
    logger.info(f"[Agent创建] 使用统一诊断智能体 (Unified Agent)")
    from .unified_agent import create_unified_diagnostic_agent
    return await create_unified_diagnostic_agent(llm, tools, checkpointer, agent_id, system_prompt)
