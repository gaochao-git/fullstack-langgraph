...
class SOPTemplate(BaseModel):
    ...
    sop_severity = Column(String(20), nullable=False, default="high", index=True)
    sop_steps = Column(JSONType, nullable=False)
    tools_required = Column(JSONType, nullable=True)
    sop_recommendations = Column(Text, nullable=False, default="")
    team_name = Column(String(100), nullable=False, default="", index=True)
